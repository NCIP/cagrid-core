...
public class UnpackPurlStep extends Step {
    ...
    public void runStep() throws Exception {
        try {
            ZipUtilities.unzip(new File(IdentifiersTestInfo.PURLZ_ZIP), testInfo.getPurlzDirectory());
        ...
        if (!System.getProperty("os.name").toLowerCase().contains("win")) {
            // make files in /bin directory executable if not on windows
            // platform
            ...
            try {
                chmodProcess = Runtime.getRuntime().exec(commandArray, null,
                    new File(testInfo.getPurlzDirectory().getAbsolutePath(), "bin"));
                new StreamGobbler(chmodProcess.getInputStream(), StreamGobbler.TYPE_OUT, System.out).start();
                new StreamGobbler(chmodProcess.getErrorStream(), StreamGobbler.TYPE_OUT, System.err).start();
                chmodProcess.waitFor();
            } catch (Exception ex) {
                throw new Exception("Error invoking chmod process: " + ex.getMessage(), ex);
            }
        }
    }
    ...
    private void writeBootloaderConfig() throws IOException {
        String bootloaderJar = testInfo.getPurlzDirectory().getAbsolutePath() + File.separator
            + IdentifiersTestInfo.PURLZ_BOOTLOADER;

        File bootloaderCfg = new File(testInfo.getPurlzDirectory().getAbsolutePath() + File.separator
            + IdentifiersTestInfo.PURLZ_BOOTLOADER_CFG);
    ...
    private void setServerPort() throws Exception {
        File serverConfigFile = new File(testInfo.getPurlzDirectory().getAbsolutePath()
        ...
        Iterator configureElementIterator = configRoot.getChildren("Configure", configRoot.getNamespace()).iterator();
        while (configureElementIterator.hasNext()) {
            ...
            while (descIterator.hasNext()) {
                ...
                if (o instanceof Element) {
                    Element e = (Element) o;
                    if (e.getName().equals("Set") && e.getAttributeValue("name").equals("Port")) {
