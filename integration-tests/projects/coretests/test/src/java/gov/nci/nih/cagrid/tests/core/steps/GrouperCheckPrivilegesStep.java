...
public class GrouperCheckPrivilegesStep extends Step {
    ...
    public void runStep() throws GridGrouperRuntimeFault, StemNotFoundFault, RemoteException, MalformedURIException {
        ...
        if (isGroup) {
            GroupPrivilege[] commonGroupPrivs = grouper.getGroupPrivileges(new GroupIdentifier(null, this.path),
                "GrouperAll");
            GroupPrivilege[] groupPrivs = grouper
                .getGroupPrivileges(new GroupIdentifier(null, this.path), this.subject);
