...
public class WebSSOSystemTest extends Story {
    ...
    protected boolean storySetUp() {
        ...
        try {
            ...
            String message = "Error creating container for dorian service: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        ...
        try {
            ...
            webSSOServiceContainer = ServiceContainerFactory.createContainer(ServiceContainerType.TOMCAT_6_CONTAINER);
            new UnpackContainerStep(webSSOServiceContainer).runStep();
        } catch (Throwable ex) {
            String message = "Error creating container for websso server: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        ...
        try {
            ...
            String message = "Error creating container for websso client jasig server: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        ...
        try {
            ...
            String message = "Error creating container for websso client acegi server: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        ...
        try {
            ...
            CopyServiceStep copyService = new CopyServiceStep(asLocation, tempDorianService);
            copyService.runStep();
        } catch (Throwable ex) {
            String message = "Error creating temp service for dorian: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        try {
            ...
            CopyServiceStep copyService = new CopyServiceStep(asLocation, tempWebSSOService);
            copyService.runStep();
        } catch (Throwable ex) {
            String message = "Error creating temp service for websso: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        ...
        try {
            ...
            CopyServiceStep copyService = new CopyServiceStep(asLocation, tempcdsService);
            copyService.runStep();
        } catch (Throwable ex) {
            String message = "Error creating temp service for cds: " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        try {
            ...
            CopyServiceStep copyService = new CopyServiceStep(asLocation, tempwebssoJasigClientService);
            copyService.runStep();
        } catch (Throwable ex) {
            String message = "Error creating temp service for websso-client : " + ex.getMessage();
            log.error(message, ex);
            fail(message);
        }
        try {
            ...
            File asLocation = new File("../../../examples/projects/websso-client-example");
            CopyServiceStep copyService = new CopyServiceStep(asLocation, tempwebssoAcegiClientService);
            copyService.runStep();
        } catch (Throwable ex) {
            String message = "Error creating temp service for websso-client : " + ex.getMessage();
            log.error(message, ex);
    ...
    protected Vector<Step> steps() {
        ...
        steps.add(new AssertWebSSOApplicationStep(webSSOClientJasigURL, webSSOClientAcegiURL + "/protected",
            httpsWebSSOPortNumber, httpsJasigPortNumber, httpsAcegiPortNumber));
    ...
    private void configureWebSSOAcegiClientServerSteps(Vector<Step> steps, List<Application> users) {
        try {
            ...
            configureWebSSOTomcatContainer(steps, users, webSSOAcegiClientServiceContainer, webssoClientHostname,
                httpsAcegiPortNumber, webssoClientServiceDir);

            String globusDirPath = System.getProperty("user.home") + File.separator + ".globus" + File.separator
                + "certificates";
            ...
            webSSOClientAcegiURL = "http://localhost:" + httpPort + "/webssoclientacegiexample-"
                + WebSSOSystemTest.getProjectVersion();

            String tomcatCertsDir = webSSOAcegiClientServiceContainer.getProperties().getContainerDirectory()
            ...
            steps.add(new ChangeAcegiCASClientPropertiesStep(tempwebssoAcegiClientService, webSSOServerURL,
                webSSOClientAcegiURL, hostCert, hostKey, httpPort, httpsAcegiPortNumber));

            String cacertsFilePath = tempwebssoAcegiClientService.getAbsolutePath() + File.separator + "ext"
                + File.separator + "dependencies-cert" + File.separator + "cert" + File.separator + "cacerts-"
                + WebSSOSystemTest.getProjectVersion() + ".cert";
            ...
            steps.add(new DeployServiceStep(webSSOAcegiClientServiceContainer, this.tempwebssoAcegiClientService
                .getAbsolutePath(), antTargets));
            steps.add(new ChangeCatalinaPropertiesStep(webSSOAcegiClientServiceContainer.getProperties(),
                "cacerts.cert"));
    ...
    private void configureWebSSOJasigClientServerSteps(Vector<Step> steps, List<Application> users) {
        try {
            ...
            configureWebSSOTomcatContainer(steps, users, webSSOJasigClientServiceContainer, webssoClientHostname,
                httpsJasigPortNumber, webssoClientServiceDir);
            ...
            String webSSOServerURL = "https://localhost:18443/webssoserver";
            webSSOClientJasigURL = "http://localhost:" + httpPort + "/webssoclientjasigexample-"
                + WebSSOSystemTest.getProjectVersion() + "/protected/";

            String tomcatCertsDir = webSSOJasigClientServiceContainer.getProperties().getContainerDirectory()
                + File.separator + "certificates";
            String hostKey = new File(tomcatCertsDir, webssoClientHostname + "-key.pem").getAbsolutePath();
            String hostCert = new File(tomcatCertsDir, webssoClientHostname + "-cert.pem").getAbsolutePath();

            steps.add(new ChangeJasigCASClientPropertiesStep(tempwebssoJasigClientService, webSSOServerURL,
                webSSOClientJasigURL));

            steps.add(new WebSSOClientCertificatesStep(tempwebssoJasigClientService, hostCert, hostKey));
            String cacertsFilePath = tempwebssoJasigClientService.getAbsolutePath() + File.separator + "ext"
                + File.separator + "dependencies-cert" + File.separator + "cert" + File.separator + "cacerts-"
                + WebSSOSystemTest.getProjectVersion() + ".cert";
            steps.add(new InstallCertStep(new File(cacertsFilePath), systemName, httpsWebSSOPortNumber, 1));
            steps.add(new DeployServiceStep(webSSOJasigClientServiceContainer, this.tempwebssoJasigClientService
                .getAbsolutePath(), antTargets));
            steps.add(new ChangeCatalinaPropertiesStep(webSSOJasigClientServiceContainer.getProperties(),
                "cacerts.cert"));
    ...
    private void configureCDSSteps(Vector<Step> steps) {
        steps.add(new DeployServiceStep(cdsServiceContainer, this.tempcdsService.getAbsolutePath()));

        File outPutDir = new File(cdsServiceContainer.getProperties().getContainerDirectory().getAbsolutePath()
            + File.separator + "certificates" + File.separator + "ca");
    ...
    private void configureWebSSOServerSteps(Vector<Step> steps, List<Application> users) {
        try {
            String webssoServerHostname = "webssoserver";
            File webssoServiceDir = webSSOServiceContainer.getProperties().getContainerDirectory();
            configureWebSSOTomcatContainer(steps, users, webSSOServiceContainer, webssoServerHostname,
                httpsWebSSOPortNumber, webssoServiceDir);
            configureWebSSO(steps, webssoServerHostname);

            steps.add(new DeployServiceStep(webSSOServiceContainer, this.tempWebSSOService.getAbsolutePath()));
    ...
    private void configureWebSSOTomcatContainer(Vector<Step> steps, List<Application> users,
        ServiceContainer tomcatContainer, String hostname, int httpsPortNumber, File serviceDir)
        ...
        steps.add(new HostCertificatesStep(tomcatCertsDir, serviceURL, userCredential, adminCredential, hostname));
    ...
    private List<Application> configureDorianSteps(Vector<Step> steps) {
        ...
        try {
            File originalPropertyFile = new File(tempDorianService.getAbsolutePath() + File.separator + "etc"
                + File.separator + "dorian.properties");
            steps.add(new ModifyConfigurationStep(originalPropertyFile, new File("resources/dorian.properties")));

            steps.add(new DeployServiceStep(dorianServiceContainer, this.tempDorianService.getAbsolutePath()));
            ...
            GridCredentialRequestStep admin = getGridAdminCredential(steps, signingCertStep, serviceURL);
    ...
    private GridCredentialRequestStep getGridAdminCredential(Vector<Step> steps,
