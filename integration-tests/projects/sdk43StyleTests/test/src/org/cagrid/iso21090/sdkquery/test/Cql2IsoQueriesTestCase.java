...
public class Cql2IsoQueriesTestCase extends TestCase {

    public static Log LOG = LogFactory.getLog(Cql2IsoQueriesTestCase.class);

    private ApplicationService sdkService = null;
    private CQL1toCQL2Converter queryConverter = null;
    private CQL2ToParameterizedHQL queryTranslator = null;
    

    public Cql2IsoQueriesTestCase(String name) {
        super(name);
    }
    ...
    public void setUp() {
        ...
        try {
            this.queryTranslator = QueryTestsHelper.getCql2Translator();
        ...
        try {
            ...
            reader.close();
            queryConverter = new CQL1toCQL2Converter(model);
        } catch (Exception ex) {
            ex.printStackTrace();
            fail("Error setting up CQL 1 to 2 converter");
        }
    }
    ...
    public void testQueryCdNullFlavorNa() {
        CQLQuery query = new CQLQuery();
        Object target = new Object();
        target.setName(CdDataType.class.getName());
        Association assoc = new Association();
    ...
    private List<?> executeQuery(CQLQuery query) {
        ...
        try {
            cql2Query = queryConverter.convertToCql2Query(query);
        } catch (QueryConversionException ex) {
            ex.printStackTrace();
            fail("Error converting CQL 1 to 2: " + ex.getMessage());
        }
        if (LOG.isDebugEnabled()) {
            StringWriter writer = new StringWriter();
            try {
                CQL2SerializationUtil.serializeCql2Query(cql2Query, writer);
        ...
        try {
            hql = queryTranslator.convertToHql(cql2Query);
        } catch (QueryTranslationException e) {
            e.printStackTrace();
            fail("Error translating CQL 2 to HQL: " + e.getMessage());
        }
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        TestResult result = runner.doRun(new TestSuite(Cql2IsoQueriesTestCase.class));
