...
/**
 * Dcql2TransferQueryExecutionStep
 * Executes a federated query using transfer for results
 *  
 * @author ervin
 */
public class Dcql2TransferQueryExecutionStep extends BaseDcql2QueryExecutionStep {
    
    private static Log LOG = LogFactory.getLog(Dcql2TransferQueryExecutionStep.class);
    ...
    public static final int WAIT_TIME = 30; // seconds
    
    private FederatedQueryProcessorClient fqpClient;
    private String[] dataServiceUrls;
    
    public Dcql2TransferQueryExecutionStep(String queryFilename, String goldFilename,
    ...
    public void runStep() throws Throwable {
        ...
        final FederatedQueryResultsRetrievalClient resultsClient = fqpClient.queryAsynchronously(query, null, null);
        /*
    ...
    private void transferAndVerify(FederatedQueryResultsRetrievalClient resultsClient) {
        ...
        */
        ...
        LOG.debug("Deserializing text from transfer service to DCQL Query Results");
        
        StringReader xmlReader = new StringReader(xml);
        DCQLQueryResultsCollection results = null;
        try {
            results = DCQL2SerializationUtil.deserializeDcql2QueryResults(xmlReader);
        } catch (Exception ex) {
            ex.printStackTrace();
            fail("Error deserializing DCQL query results: " + ex.getMessage());
        }
        
        // validate
        LOG.debug("VALIDATING RESULTS");
        DCQLQueryResultsCollection dcqlGoldResults = loadGoldDcqlResults();
        QueryResultsVerifier.verifyDcql2Results(results, dcqlGoldResults);
