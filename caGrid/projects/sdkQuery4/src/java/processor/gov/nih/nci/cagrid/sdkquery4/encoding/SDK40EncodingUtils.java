...
public class SDK40EncodingUtils {
	...
	protected static Log LOG = LogFactory.getLog(SDK40EncodingUtils.class.getName());
	...
	private static Mapping getMapping(MessageContext context, String mappingProperty) {
		...
		EntityResolver resolver = new EntityResolver() {
			public InputSource resolveEntity(String publicId, String systemId) {
				if (publicId.equals(CASTOR_MAPPING_DTD_ENTITY)) {
					InputStream in = ClassUtils.getResourceAsStream(SDK40EncodingUtils.class, CASTOR_MAPPING_DTD);
		...
		if (mappingCacheMap.containsKey(mappingLocation)) {
			...
			InputStream mappingStream = ClassUtils.getResourceAsStream(SDK40EncodingUtils.class, mappingLocation);
