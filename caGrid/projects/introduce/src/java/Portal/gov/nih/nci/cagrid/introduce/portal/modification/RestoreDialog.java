    ...
    private JComboBox getBackupsComboBox() {
        if (backupsComboBox == null) {
            ...
            for (int i = 0; i < files.length; i++) {
                StringTokenizer strtok = new StringTokenizer(files[i], "_", false);
                strtok.nextToken();
            ...
            restoreButton.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    ...
                    if (getBackupsComboBox().getItemCount() == 0) {
                        ErrorDialog.showError("No selected or available services in cache");
                        return;
                    }

                    int decision = JOptionPane.showConfirmDialog(RestoreDialog.this,
                        "Are you sure you wish to roll back?\n" + "This will roll back to the last save point!\n"
                            ...
                            + "and click Modify Service again.", "Are you sure", JOptionPane.YES_NO_OPTION);
                        ...
                        BusyDialogRunnable r = new BusyDialogRunnable(
                            ...
                            public void process() {
                                try {
                                    ...
                                    ResourceManager.restoreSpecific(String.valueOf(((Date) getBackupsComboBox()
                                        .getSelectedItem()).getTime()), serviceName, baseDir);
                                    dispose();
                                } catch (Exception e1) {
                                    CompositeErrorDialog.showErrorDialog("Unable to roll back: " + e1.getMessage());
                                    dispose();
                                    return;
                                }
                            }
