...
public class ExtensionsUpgradeManager {
    ...
    public boolean needsUpgrading() {
        boolean needsUpgrading = false;
        if (needsRemoving()) {
            // bail out early if the extension needs removed
            return true;
        }
    ...
    private boolean needsRemoving() {
        ...
        if (extensions != null) {
            for (int extensionI = 0; extensionI < extensions.length; extensionI++) {
                ExtensionType extension = extensions[extensionI];
                ExtensionDescription extDescription = ExtensionsLoader.getInstance().getExtension(extension.getName());
                if (extDescription != null && extDescription.getServiceExtensionDescription() != null) {
            ...
            if (extDescription != null) {
                if ((extDescription.getVersion() != null)) {
                    ...
                    for (int i = 0; i < upgrades.size(); i++) {
                        ...
                        Constructor con = clazz.getConstructor(new Class[]{ExtensionType.class,
                                ServiceInformation.class, String.class, String.class, String.class});
