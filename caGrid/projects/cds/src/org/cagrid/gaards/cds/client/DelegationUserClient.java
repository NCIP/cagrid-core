 ...
 * 
 * A client to simplify interaction with the CDS
 */
public class DelegationUserClient {
    ...
    public DelegatedCredentialReference delegateCredential(DelegationPolicy policy,
        ProxyLifetime issuedCredentialLifetime) throws RemoteException, CDSInternalFault, DelegationFault,
     ...
     * @throws URI.MalformedURIException
     */
    public DelegatedCredentialReference delegateCredential(ProxyLifetime delegationLifetime, DelegationPolicy policy,
        ProxyLifetime issuedCredentialLifetime) throws RemoteException, CDSInternalFault, DelegationFault,
        PermissionDeniedFault, URI.MalformedURIException {
        return this.delegateCredential(delegationLifetime, 1, policy, issuedCredentialLifetime, 0,
     ...
     * @throws URI.MalformedURIException
     */
    public DelegatedCredentialReference delegateCredential(ProxyLifetime delegationLifetime, int delegationPathLength,
        DelegationPolicy policy, ProxyLifetime issuedCredentialLifetime, int issuedCredentialPathLength, int keyLength)
        throws RemoteException, CDSInternalFault, DelegationFault, PermissionDeniedFault, URI.MalformedURIException {
        ...
        try {
            PublicKey publicKey = KeyUtil.loadPublicKey(dsr.getPublicKey().getKeyAsString());

            X509Certificate[] certs = ProxyCreator.createImpersonationProxyCertificate(cred.getCertificateChain(), cred
                .getPrivateKey(), publicKey, hours, minutes, seconds, delegationPathLength);
            chain = Utils.toCertificateChain(certs);
        } catch (Exception e) {
            DelegationFault f = new DelegationFault();
            f.setFaultString("Unexpected error creating proxy: " + e.getMessage() + ".");
    ...
    public List<DelegationRecord> findMyDelegatedCredentials() throws RemoteException, CDSInternalFault,
    ...
    public List<DelegationRecord> findMyDelegatedCredentials(DelegationRecordFilter filter) throws RemoteException,
        CDSInternalFault, DelegationFault, PermissionDeniedFault {
        if (filter == null) {
            filter = new DelegationRecordFilter();
        }
        if (cred != null) {
            filter.setGridIdentity(CertUtil.getIdentity(cred));
        } else {
            try {
                GlobusCredential c = ProxyUtil.getDefaultProxy();
                if (c != null) {
                    filter.setGridIdentity(CertUtil.getIdentity(c));
                }
            } catch (Exception e) {
                DelegationFault f = new DelegationFault();
        ...
        if (filter.getGridIdentity() == null) {
            throw Errors.getPermissionDeniedFault(Errors.AUTHENTICATION_REQUIRED);
        }
    ...
    public void suspendDelegatedCredential(DelegationIdentifier id) throws RemoteException, CDSInternalFault,
        DelegationFault, PermissionDeniedFault {
    ...
    public List<DelegationDescriptor> findCredentialsDelegatedToClient() throws RemoteException, CDSInternalFault,
    ...
    public List<DelegationDescriptor> findCredentialsDelegatedToClient(String ownerGridIdentity)
    ...
    public List<DelegationDescriptor> findCredentialsDelegatedToClient(ClientDelegationFilter filter)
        throws RemoteException, CDSInternalFault, PermissionDeniedFault {
        DelegationDescriptor[] results = client.findCredentialsDelegatedToClient(filter);
    ...
    public List<DelegatedCredentialAuditRecord> searchDelegatedCredentialAuditLog(DelegatedCredentialAuditFilter f)
        throws RemoteException, CDSInternalFault, DelegationFault, PermissionDeniedFault {
        DelegatedCredentialAuditRecord[] r = client.searchDelegatedCredentialAuditLog(f);
