...
/**
 * WsEnumUpgradeFrom1pt0
 * Upgrades caGrid ws-enumeration support from 1.0 to current
 * 
 * @author ervin
 ...
 *          dervin Exp $
 */
public class WsEnumUpgradeFrom1pt0 extends ExtensionUpgraderBase {
    ...
    private static Log LOG = LogFactory.getLog(WsEnumUpgradeFrom1pt0.class);

    public WsEnumUpgradeFrom1pt0(ExtensionType extensionType, ServiceInformation serviceInfo, String servicePath,
        String fromVersion, String toVersion) {
        super(WsEnumUpgradeFrom1pt0.class.getSimpleName(),
    ...
    private void upgradeJars() {
        FileFilter enumLibFilter = new FileFilter() {
            public boolean accept(File pathname) {
                ...
                && (name.startsWith(CAGRID_1_0_WS_ENUM_JAR_PREFIX));
            }
        };
        ...
        for (File oldLib : serviceEnumLibs) {
            oldLib.delete();
            String description = "caGrid 1.0 library " + oldLib.getName() + " removed";
        ...
        if (deleteJar1.exists()) {
            deleteJar1.delete();
            String description = "caGrid 1.0 library " + deleteJar1.getName() + " removed";
            getStatus().addDescriptionLine(description);
            LOG.debug(description);
        }
        if (deleteJar2.exists()) {
            deleteJar2.delete();
            String description = "caGrid 1.0 library " + deleteJar2.getName() + " removed";
            getStatus().addDescriptionLine(description);
            System.out.println(description);
        }
