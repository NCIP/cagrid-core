...
public class DefaultSAMLProvider implements org.cagrid.gaards.authentication.service.SAMLProvider {
    ...
    private static Log LOG = LogFactory.getLog(DefaultSAMLProvider.class);
    ...
    public void loadCertificates() {
        ...
        try {
            ...
            String message = "Error loading certificates: " + ex.getMessage();
    ...
    public SAMLAssertion getSAML(Subject subject) throws InsufficientAttributeException {
        LOG.debug("Getting SAML assertion");
        // Get attributes
        String uid = null;
        ...
        while (it.hasNext()) {
            ...
            if (p instanceof LoginIdPrincipal) {
                uid = p.getName();
            } else if (p instanceof FirstNamePrincipal) {
                firstName = p.getName();
            } else if (p instanceof LastNamePrincipal) {
                lastName = p.getName();
            } else if (p instanceof EmailIdPrincipal) {
                email = p.getName();
            }
        }
        ...
        try {
            ...
            SAMLAuthenticationStatement auth = new SAMLAuthenticationStatement(sub,
                "urn:oasis:names:tc:SAML:1.0:am:unspecified", new Date(), ipAddress, subjectDNS, null);
            ...
            String message = "Error creating SAML assertion: " + e.getMessage();
            LOG.error(message, e);
