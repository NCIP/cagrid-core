 ...
 *          Exp $
 */
public class IdentityFederationManager extends LoggingObject implements
		Publisher {
	...
	public IdentityFederationManager(IdentityFederationProperties conf,
			Database db, PropertyManager properties, CertificateAuthority ca,
			EventManager eventManager, FederationDefaults defaults,
			boolean ignoreCRL) throws DorianInternalFault {
		super();
		this.conf = conf;
		this.ca = ca;
		this.db = db;
		...
		try {
			...
			this.eventManager.logEvent(AuditConstants.SYSTEM_ID,
					AuditConstants.SYSTEM_ID, FederationAudit.InternalError
							.getValue(), mess + "\n\n"
	...
	private void initializeEventManager() throws DorianInternalFault {
		try {
			...
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.AccessDenied.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));

			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.IdPAdded.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.IdPUpdated.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.IdPRemoved.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.AdminAdded.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.AdminRemoved.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.CRLPublished.getValue(),
							AuditingConstants.FEDERATION_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.AccountCreated.getValue(),
							AuditingConstants.GRID_ACCOUNT_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.AccountUpdated.getValue(),
							AuditingConstants.GRID_ACCOUNT_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.AccountRemoved.getValue(),
							AuditingConstants.GRID_ACCOUNT_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
			...
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.HostCertificateRequested.getValue(),
							AuditingConstants.HOST_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.HostCertificateApproved.getValue(),
							AuditingConstants.HOST_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
			...
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.HostCertificateRenewed.getValue(),
							AuditingConstants.HOST_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.UserCertificateUpdated.getValue(),
							AuditingConstants.USER_CERTIFICATE_AUDITOR));
			this.eventManager
					.registerEventWithHandler(new EventToHandlerMapping(
							FederationAudit.UserCertificateRemoved.getValue(),
							AuditingConstants.USER_CERTIFICATE_AUDITOR));
		} catch (Exception e) {
	...
	public GridUserPolicy[] getUserPolicies(String callerGridIdentity)
		...
		try {
			...
			this.eventManager.logEvent(callerGridIdentity,
					AuditConstants.SYSTEM_ID, FederationAudit.AccessDenied
							.getValue(), mess + "\n\n"
							+ Utils.getExceptionMessage(e));
			throw e;
		}
	...
	public void updateTrustedIdP(String callerGridIdentity, TrustedIdP idp)
		...
		try {
			...
			this.eventManager.logEvent(idp.getName(), callerGridIdentity,
					FederationAudit.IdPUpdated.getValue(), ReportUtils
			...
			for (int i = 0; i < users.length; i++) {
				try {
					...
					this.eventManager.logEvent(users[i].getGridId(),
							callerGridIdentity, FederationAudit.AccountRemoved
	...
	public void updateUser(String callerGridIdentity, GridUser usr)
		...
		try {
			...
			this.eventManager.logEvent(curr.getGridId(), callerGridIdentity,
					FederationAudit.AccountUpdated.getValue(), ReportUtils
	...
	private void removeUser(GridUser usr) throws DorianInternalFault,
			InvalidUserFault {
		try {
			um.removeUser(usr);
			...
			for (int i = 0; i < records.size(); i++) {
				...
				if ((r.getStatus().equals(HostCertificateStatus.Active))
					...
					this.eventManager.logEvent(String.valueOf(r.getId()),
							AuditConstants.SYSTEM_ID,
							FederationAudit.HostCertificateUpdated.getValue(),
							"The status of the certificate for the host "
									...
									+ " to "
									+ HostCertificateStatus.Compromised
											.getValue()
					...
					this.eventManager.logEvent(String.valueOf(r.getId()),
							...
							"The status of the certificate for the host "
									+ r.getHost() + " (" + r.getId()
									+ "), was changed from "
									+ r.getStatus().getValue() + " to "
									+ HostCertificateStatus.Rejected.getValue()
									+ ", because its owner's (" + r.getOwner()
									+ ") account was removed.");
				}
			}
	...
	public void removeUser(String callerGridIdentity, GridUser usr)
		...
		try {
			...
			this.eventManager.logEvent(usr.getGridId(), callerGridIdentity,
					FederationAudit.AccountRemoved.getValue(), usr
							.getFirstName()
							+ " "
							+ usr.getLastName()
	...
	public X509Certificate requestUserCertificate(SAMLAssertion saml,
			...
			throws DorianInternalFault, InvalidAssertionFault, UserPolicyFault,
			PermissionDeniedFault {
		TrustedIdP idp = null;
		try {
			...
			this.eventManager.logEvent(AuditConstants.SYSTEM_ID,
					AuditConstants.SYSTEM_ID,
					FederationAudit.InvalidUserCertificateRequest.getValue(),
					Utils.getExceptionMessage(e));
			throw e;
		}
		...
		try {
			gid = UserManager.subjectToIdentity(UserManager.getUserSubject(
		...
		try {
			...
			this.eventManager
					.logEvent(usr.getGridId(), AuditConstants.SYSTEM_ID,
							FederationAudit.SuccessfulUserCertificateRequest
									.getValue(), "User certificate ("
	...
	public HostCertificateRecord requestHostCertificate(String callerGridId,
		...
		try {
			...
			this.eventManager.logEvent(callerGridId, AuditConstants.SYSTEM_ID,
					FederationAudit.AccessDenied.getValue(), mess + "\n\n"
							+ Utils.getExceptionMessage(e));
			throw e;
		}
	}
	...
	public void updateHostCertificateRecord(String callerGridId,
		...
		try {
			...
			this.eventManager.logEvent(String.valueOf(record.getId()),
					callerGridId, FederationAudit.HostCertificateUpdated
	...
	private GridUser getUser(String gridId) throws DorianInternalFault,
			PermissionDeniedFault {
		try {
	...
	private void verifyAdminUser(GridUser usr) throws DorianInternalFault,
			PermissionDeniedFault {
		try {
	...
	private void verifyActiveUser(GridUser usr) throws DorianInternalFault,
			PermissionDeniedFault {
	...
	public void clearDatabase() throws DorianInternalFault {
		...
		try {
			...
			DorianInternalFault fault = new DorianInternalFault();
			fault
					.setFaultString("An unexpected error occurred in deleting the auditing logs.");
			FaultHelper helper = new FaultHelper(fault);
			helper.addFaultCause(e);
			fault = (DorianInternalFault) helper.getFault();
			throw fault;
	...
	public void removeUserCertificate(String callerIdentity, long serialNumber)
		...
		try {
			...
			this.eventManager.logEvent(String.valueOf(serialNumber),
					callerIdentity, FederationAudit.UserCertificateRemoved
	...
	public List<GridUserRecord> userSearch(String callerIdentity,
		...
		try {
			...
			if (conf.getUserSearchPolicy().equals(
					IdentityFederationProperties.ADMIN_SEARCH_POLICY)) {
				GridUser caller = getUser(callerIdentity);
				verifyActiveUser(caller);
				verifyAdminUser(caller);
			} else if (conf.getUserSearchPolicy().equals(
					IdentityFederationProperties.AUTHENTICATED_SEARCH_POLICY)) {
