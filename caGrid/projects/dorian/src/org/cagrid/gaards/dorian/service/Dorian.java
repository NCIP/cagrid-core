...
public class Dorian extends LoggingObject {
	...
	public Dorian(DorianProperties conf, String serviceId, boolean ignoreCRL)
		...
		try {
			...
			this.ifm = new IdentityFederationManager(ifsConfiguration, db,
					properties, ca, this.eventManager, defaults, ignoreCRL);
	...
	public LocalUser[] findLocalUsers(String gridIdentity,
			LocalUserFilter filter) throws DorianInternalFault,
			PermissionDeniedFault {
		String uid = null;
		try {
			uid = ifm.getUserIdVerifyTrustedIdP(identityProvider
					.getIdPCertificate(), gridIdentity);
		} catch (Exception e) {
			String message = "Permission to find local users was denied, caller is not a valid user.";
			this.eventManager
					.logEvent(gridIdentity, AuditConstants.SYSTEM_ID,
							IdentityProviderAudit.LocalAccessDenied.getValue(),
							message);
	...
	public void removeLocalUser(String gridIdentity, String userId)
		...
		this.ifm.removeUserByLocalIdIfExists(identityProvider
	...
	public String registerLocalUser(Application a) throws DorianInternalFault,
			InvalidUserPropertyFault {
