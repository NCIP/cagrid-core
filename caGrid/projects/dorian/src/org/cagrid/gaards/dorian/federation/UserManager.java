...
public class UserManager extends LoggingObject {
	...
	public synchronized List<GridUserRecord> getUsers(
			GridUserSearchCriteria filter) throws DorianInternalFault {
		...
		try {
			...
			if (filter != null) {
				s = c.prepareStatement("select " + GID_FIELD + ","
						...
						+ GID_FIELD + " LIKE ? AND " + FIRST_NAME_FIELD + " LIKE ? AND "
						+ LAST_NAME_FIELD + " LIKE ? AND " + EMAIL_FIELD
						+ " LIKE ?");
		...
		if (!determineIfUserExists(user.getIdPId(), user.getUID())) {
			...
			try {
				...
				String caSubject = ca.getCACertificate().getSubjectDN()
						.getName();
				user.setGridId(subjectToIdentity(getUserSubject(caSubject, idp,
						user.getUID())));
	...
	public synchronized void updateUser(GridUser u) throws DorianInternalFault,
			InvalidUserFault {
		this.buildDatabase();
			...
			try {
				...
				if ((u.getUserStatus() != null)
					...
					if (accountCreated(curr.getUserStatus())
						...
						InvalidUserFault fault = new InvalidUserFault();
						fault
								.setFaultString("Cannot change the status of account that has been rejected.");
						throw fault;
					}
				...
				fault.setFaultString("Error updating the Grid user "
						+ getCredentialsManagerUID(u.getIdPId(), u.getUID())
	...
	private void validateSpecifiedField(String type, String name,
			boolean validateLength) throws InvalidUserFault {
