...
public class UserManager extends LoggingObject {
    ...
    public synchronized GridUserRecord getUserRecord(String gridId) throws DorianInternalFault, InvalidUserFault {
        this.buildDatabase();
        GridUserRecord user = new GridUserRecord();
        Connection c = null;
        try {
            c = db.getConnection();
            PreparedStatement s = c.prepareStatement("select " + GID_FIELD + "," + FIRST_NAME_FIELD + ","
                + LAST_NAME_FIELD + "," + EMAIL_FIELD + " from " + USERS_TABLE + " WHERE " + GID_FIELD + "= ?");
            s.setString(1, gridId);
            ResultSet rs = s.executeQuery();
            if (rs.next()) {
    ...
    public synchronized List<GridUserRecord> getUsers(GridUserSearchCriteria filter) throws DorianInternalFault {
