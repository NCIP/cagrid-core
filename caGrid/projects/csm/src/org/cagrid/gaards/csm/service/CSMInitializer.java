...
public class CSMInitializer {
	...
	public static AuthorizationManager getAuthorizationManager(
		...
		try {
			...
			Application webService = getApplication(sf,
			...
			if (webService == null) {
				...
				webService
						.setApplicationName(Constants.CSM_WEB_SERVICE_CONTEXT);
				webService
						.setApplicationDescription(Constants.CSM_WEB_SERVICE_DESCRIPTION);
			...
			AuthorizationManager auth = (AuthorizationManager) SecurityServiceProvider
			...
			CSMInternalFault fault = new CSMInternalFault();
			fault
					.setFaultString("Unexpected error initializing the CSM Web Service.");
			throw fault;
		}
	}

	private static void createAdminGroupIfNeeded(AuthorizationManager auth,
			Application webService) throws CSMInternalFault {
		Group grp = new Group();
		grp.setApplication(webService);
		grp.setGroupName(webService.getApplicationName() + " "
				+ Constants.ADMIN_GROUP_SUFFIX);
		List<ProtectionGroup> grps = auth.getObjects(new GroupSearchCriteria(
				grp));
		if (grps.size() == 0) {
	...
	private static void createAdministratorRoleIfNeeded(
			AuthorizationManager auth, Application webService)
			throws CSMInternalFault {
		Privilege priv = new Privilege();
		priv.setName(Constants.ADMIN_PRIVILEGE);
		List<Privilege> privileges = auth
				.getObjects(new PrivilegeSearchCriteria(priv));
		...
		if (roles.size() == 0) {
			...
			try {
				...
				CSMInternalFault fault = new CSMInternalFault();
				fault
						.setFaultString("Error initializing the CSM Web Service, could not create the "
	...
	private static void createAdminPrivilegeIfNeeded(AuthorizationManager auth)
			throws CSMInternalFault {
		Privilege priv = new Privilege();
		priv.setName(Constants.ADMIN_PRIVILEGE);
		List<Privilege> privs = auth.getObjects(new PrivilegeSearchCriteria(
				priv));
		if (privs.size() == 0) {
	...
	private static void initializeApplications(AuthorizationManager auth,
			Application webService) throws CSMInternalFault {
		List<Application> apps = auth.getObjects(new ApplicationSearchCriteria(
				new Application()));
	...
	public static void initializeApplication(AuthorizationManager auth,
			Application webService, Application a) throws CSMInternalFault {
		...
		if (pes.size() == 0) {
			logInfo("No protection element found for application "
					+ a.getApplicationName() + ".");
			app
					.setProtectionElementDescription("CSM Web Service protection element for the application "
							+ a.getApplicationName() + ".");
			try {
			...
			logInfo("Created protection element for application "
					+ a.getApplicationName() + ".");
		}
		...
		if (pgs.size() == 0) {
			logInfo("No protection group found for application "
					+ a.getApplicationName() + ".");
			addMapping = true;
			pg
					.setProtectionGroupDescription("CSM Web Service protection group for the application "
							+ a.getApplicationName() + ".");
			try {
		...
		if (grps.size() == 0) {
			logInfo("No administrators group found for application "
					+ a.getApplicationName() + ".");
			addMapping = true;
			grp
					.setGroupDesc("CSM Web Service administrator group for the application "
							+ a.getApplicationName() + ".");
			try {
				auth.createGroup(grp);
			} catch (Exception e) {
			...
			logInfo("Successfully created an administrators group ("
					+ grp.getGroupName() + ") for application "
					+ a.getApplicationName() + ".");
		}

		if (addMapping) {
			logInfo("Access control rights on the application "
					+ a.getApplicationName()
			...
			List<ProtectionGroup> protectionGroups = auth
					.getObjects(new ProtectionGroupSearchCriteria(
							protectionGroup));
			...
			try {
				auth.assignGroupRoleToProtectionGroup(String
						...
						String.valueOf(group.getGroupId()), rls);
				logInfo("Successfully assigned the members of the group "
						+ group.getGroupName() + ", the role " + r.getName()
				...
				fault
						.setFaultString("Error initializing the application, "
								+ a.getApplicationName()
								+ " could not assign members of the group "
								+ group.getGroupName() + " the role "
								+ r.getName() + " on the application "
								+ a.getApplicationName() + ".");
				throw fault;
			}
			...
			if (!a.getApplicationName().equals(
				...
				try {
					...
					logInfo("Successfully assigned the members of the group "
							+ wheel.getGroupName() + ", the role "
							+ r.getName() + ", on the application "
							+ a.getApplicationName() + ".");
				} catch (Exception e) {
					logError(e.getMessage(), e);
					CSMInternalFault fault = new CSMInternalFault();
					fault.setFaultString("Error initializing the application, "
							+ a.getApplicationName()
							+ " could not assign members of the group "
							+ wheel.getGroupName() + " the role " + r.getName()
							+ " on the application " + a.getApplicationName()
							+ ".");
					throw fault;
				}
	...
	private static void createObject(SessionFactory sf, Object obj)
		...
		try {
			...
			throw new CSTransactionException(
					"An error occured in creating the "
							...
							+ pve.getMessage().substring(
									pve.getMessage().indexOf(":")), pve);
			...
			throw new CSTransactionException(
					"An error occured in creating the "
							+ StringUtilities.getClassName(obj.getClass()
									.getName())
							+ ".\n"
							+ " Duplicate entry was found in the database for the entered data",
					cve);
		} catch (Exception ex) {
	...
	private static Application getApplication(SessionFactory sf,
			String contextName) throws CSObjectNotFoundException {
		Session s = null;
		...
		try {
			...
			Query q = s
					.createQuery("from Application as app where app.applicationName='"
							+ contextName + "'");
			List list = q.list();
			...
			throw new CSObjectNotFoundException(
					" Invalid database login credentials in the application hibernate configuration file.",
					eex);
