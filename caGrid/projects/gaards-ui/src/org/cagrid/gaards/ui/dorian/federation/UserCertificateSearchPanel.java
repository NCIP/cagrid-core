...
public class UserCertificateSearchPanel extends JPanel implements DorianSessionProvider {
    ...
    public UserCertificateSearchPanel(DorianSession session, boolean administratorMode) {
        this(session, null, administratorMode);
    }


    public UserCertificateSearchPanel(DorianSession session, String gridId, boolean administratorMode) {
        super();
    ...
    private JPanel getSearchCriteriaPanel() {
        if (searchCriteriaPanel == null) {
            ...
            if (allowGridIdentitySearch) {
                searchCriteriaPanel.add(getGridIdentityPanel(), gridBagConstraints3);
            }
            searchCriteriaPanel.add(getButtonPanel(), gridBagConstraints10);
            searchCriteriaPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Search Criteria",
    ...
    private JPanel getOtherCriteria() {
        if (otherCriteria == null) {
            ...
            otherCriteria.add(getUserCertificateSerialNumber(), gridBagConstraints31);
    ...
    private void removeCertificate() {
        try {
            ...
            if ((row >= 0) && (row < getUserCertificates().getRowCount())) {
                UserCertificateRecord record = (UserCertificateRecord) getUserCertificates().getValueAt(row, 0);
                ...
                if (selection == JOptionPane.YES_OPTION) {
                    ...
                    GridApplication.getContext().showMessage("The selected certificates was successfully removed!!!");
        ...
        try {
            ...
            if (selection == JOptionPane.YES_OPTION) {
                ...
                for (int i = 0; i < rowCount; i++) {
                    ...
                    UserCertificateRecord record = (UserCertificateRecord) getUserCertificates().getValueAt(0, 0);
                    client.removeUserCertificate(record.getSerialNumber());
                    getUserCertificates().removeRow(0);
                }
                stopProgess(rowCount + " certificate(s) removed.");
                GridApplication.getContext().showMessage("The listed certificates were successfully removed!!!");
            }
        } catch (Exception e) {
            stopProgess("Error");
            ErrorDialog.showError(e);
