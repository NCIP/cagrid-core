...
public class Installer
{
	...
	private static void rightJavaType()
	{
		String java = "java";
		if (InstallerUtils.isWindows()) java += ".exe";
		String error = "nothing here";
		try
		{
			Process p = Runtime.getRuntime().exec(new String[] { InstallerUtils.getJavaHomePath() + "/bin/" + java, "-version" });
			StringBuffer stdout = new StringBuffer();
			new IOThread(p.getInputStream(), System.out, stdout).start();
			StringBuffer stderr = new StringBuffer();
			new IOThread(p.getErrorStream(), System.err, stderr).start();
			int code = p.waitFor();
			String kind = stdout.toString();
			if (InstallerUtils.isEmpty(kind)) kind = stderr.toString();
			kind = kind.toLowerCase();
			if (kind.contains("openjdk"))
			{
				error = "ERROR: HAVE WRONG KIND OF JAVA (OPEN JDK) \nUSE SUN JDK TO INSTALL AND RUN CAGRID";
			}
			
			if (kind.contains("ibm j9"))
			{
				error = "ERROR: HAVE WRONG KIND OF JAVA (IBM J9) \nUSE SUN JDK TO INSTALL AND RUN CAGRID";
			}
	...
	{
		ImageIcon myImage = new ImageIcon(Thread.currentThread().getContextClassLoader().getResource("images/installer.1.4.gif"));
	...
	private void initSteps(Map<String, String> defaultState)
	{
		...
		// Initialize steps
		PresentLicenseStep licenseStep = new PresentLicenseStep(this.model.getMessage("accept.license.title"), this.model.getMessage("accept.license.desc"));
		...
		// any of the above.
		SelectInstallationTypeStep selectInstallStep = new SelectInstallationTypeStep(this.model.getMessage("select.install.title"), this.model.getMessage("select.install.desc"));
		selectInstallStep.getOptions().add(new BooleanPropertyConfigurationOption(Constants.INSTALL_CONFIGURE_CAGRID, this.model.getMessage("select.install.configure.cagrid"), true, true));
		selectInstallStep.getOptions().add(new BooleanPropertyConfigurationOption(Constants.INSTALL_CONFIGURE_CONTAINER, this.model.getMessage("select.install.configure.container"), false, true));
	...
	{
		final RunTasksStep installDependenciesStep = new RunTasksStep(this.model.getMessage("install.dependencies.title"), this.model.getMessage("install.dependencies.desc"));
		...
		Condition shouldInstallcaGrid = new Condition()
		{
			public boolean evaluate(WizardModel m)
			{
				...
				return installDependenciesStep.getTasksCount(model) > 0 || (model.isTrue(Constants.INSTALL_CONFIGURE_CAGRID) && installDependenciesStep.getTasksCount(model) > 0);
			}
		};
	...
	private void addConfigurecaGridSteps()
	{
		...
		PropertyConfigurationStep checkReconfigureCaGridStep = new PropertyConfigurationStep(model.getMessage("check.reconfigure.cagrid.title"), model.getMessage("check.reconfigure.cagrid.desc"));
		checkReconfigureCaGridStep.getOptions().add(new BooleanPropertyConfigurationOption(Constants.RECONFIGURE_CAGRID, model.getMessage("yes"), false, false));
		...
		TargetGridConfigurationStep confStep = new TargetGridConfigurationStep(model.getMessage("select.target.grid.title"), model.getMessage("select.target.grid.desc"));
		...
		final RunTasksStep tasksStep = new RunTasksStep(model.getMessage("configuring.target.grid"), model.getMessage("configuring.target.grid"));
		ConfigureTargetGridTask configTargetGridTask = new ConfigureTargetGridTask(model.getMessage("configuring.target.grid"), model.getMessage("configuring.target.grid"));
		...
		tasksStep.getTasks().add(new ConditionalTask(configTargetGridTask, new Condition()
		{
			...
			{
				CaGridInstallerModel model = (CaGridInstallerModel) m;
				return (model.isTrue(Constants.RECONFIGURE_CAGRID) || model.isTrue(Constants.REINSTALL_CAGRID) || !model.isCaGridInstalled()) && (model.getProperty(Constants.TARGET_GRID) != null) && !model.getProperty(Constants.TARGET_GRID).equals(Constants.NO_TARGET_GRID);
		...
		PreviewTasksStep previewTasks = new PreviewTasksStep(model.getMessage("configuring.target.grid"), model.getMessage("configuring.target.grid"), tasksStep, model);
	...
	private void addConfigureContainerSteps()
	{
		...
		PropertyConfigurationStep selectContainerStep = new PropertyConfigurationStep(this.model.getMessage("select.container.title"), this.model.getMessage("select.container.desc"));
		selectContainerStep.getOptions().add(new ListPropertyConfigurationOption(Constants.CONTAINER_TYPE, this.model.getMessage("container.type"), new String[] { this.model.getMessage("container.type.tomcat"), this.model.getMessage("container.type.jboss") }, true));
		selectContainerStep.getOptions().add(new BooleanPropertyConfigurationOption(Constants.USE_SECURE_CONTAINER, this.model.getMessage("globus.check.secure.desc"), false, false));
		...
		SpecifyPortsStep containerConfigureStep = new SpecifyPortsStep(this.model.getMessage("specify.ports.title"), this.model.getMessage("specify.ports.desc"));
		...
		containerConfigureStep.getOptions().add(new TextPropertyConfigurationOption(Constants.SERVICE_HOSTNAME, this.model.getMessage("service.hostname"), hostname, true));
		containerConfigureStep.getOptions().add(new TextPropertyConfigurationOption(Constants.SHUTDOWN_PORT, this.model.getMessage("shutdown.port"), this.model.getProperty(Constants.SHUTDOWN_PORT), false));
		containerConfigureStep.getOptions().add(new TextPropertyConfigurationOption(Constants.HTTP_PORT, this.model.getMessage("http.port"), this.model.getProperty(Constants.HTTP_PORT), model.isTrue(Constants.USE_SECURE_CONTAINER)));
		containerConfigureStep.getOptions().add(new TextPropertyConfigurationOption(Constants.HTTPS_PORT, this.model.getMessage("https.port"), this.model.getProperty(Constants.HTTPS_PORT), model.isTrue(Constants.USE_SECURE_CONTAINER)));
		...
		PropertyConfigurationStep selectHostCertConfigurationStep = new PropertyConfigurationStep(this.model.getMessage("configure.host.creds.title"), this.model.getMessage("configure.host.creds.desc"));
		...
		values[2] = new LabelValuePair("Host credentials are already installed.", Constants.HOST_CREDS_ALREADY_INSTALLED);
		selectHostCertConfigurationStep.getOptions().add(new ListPropertyConfigurationOption(Constants.HOST_CREDS_SELECTION_TYPE, "Obtain host credentials method", values));
		...
		model.add(gaardsForHostCreds, new Condition()
		{
			...
			{
				return model.getProperty(Constants.HOST_CREDS_SELECTION_TYPE) != null && model.getProperty(Constants.HOST_CREDS_SELECTION_TYPE).equals(Constants.HOST_CREDS_FROM_GAARDS) && model.isConfigureContainerSelected() && model.isTrue(Constants.USE_SECURE_CONTAINER);
			}
		});
		...
		model.add(browseToHostCreds, new Condition()
		{
			...
			public boolean evaluate(WizardModel arg0)
			{
				return model.getProperty(Constants.HOST_CREDS_SELECTION_TYPE) != null && model.getProperty(Constants.HOST_CREDS_SELECTION_TYPE).equals(Constants.HOST_CREDS_FROM_BROWSE) && model.isConfigureContainerSelected() && model.isTrue(Constants.USE_SECURE_CONTAINER);
			}
		});
	...
	{
		final RunTasksStep deployContainer = new RunTasksStep(this.model.getMessage("install.title"), this.model.getMessage("install.desc"));
	...
	public void run()
	{
		ImageIcon myImage = new ImageIcon(Thread.currentThread().getContextClassLoader().getResource("images/caGrid_small.png"));
