...
import gov.nih.nci.cagrid.fqp.stubs.FederatedQueryProcessorResourceProperties;
...
public abstract class FederatedQueryProcessorResourceBase extends ReflectionResource implements Resource
    ...
    public FederatedQueryProcessorResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((FederatedQueryProcessorResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(FederatedQueryProcessorConstants.SERVICEMETADATA);
	...
	public FederatedQueryProcessorResourceConfiguration getConfiguration() {
		...
		servicePath+="/FederatedQueryProcessor";
		...
		try {
			...
			this.configuration = (FederatedQueryProcessorResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((FederatedQueryProcessorResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
